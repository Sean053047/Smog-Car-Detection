...
class STrack(BaseTrack, Track):
    ...
    def __init__(self, tlwh, score, cls_id):
        ...
        self.bboxes = dict()
        self.cls_ids = [np.asarray(cls_id).tolist()]
        self.cls_id = None
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        self.bboxes[self.frame_id]= self.tlbr
    ...
    def update(self, new_track, frame_id):
        ...
        self.score = new_track.score
        self.cls_ids += new_track.cls_ids
        self.bboxes[self.frame_id] = self.tlbr
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        cls_second = cls_ids[inds_second]

        dets = bboxes[remain_inds]
        scores_keep = scores[remain_inds]
        cls_keep = cls_ids[remain_inds]    
        
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls_id) for
                          (tlbr, s, cls_id) in zip(dets, scores_keep, cls_keep)]
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls_id) for
                          (tlbr, s, cls_id) in zip(dets_second, scores_second, cls_second)]
        else:
            detections_second = []
